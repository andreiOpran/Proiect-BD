...
-- AFISEAZA VEHICULELE (ID_VEHICUL, MARCA, MODEL, PRET) DIN SHOWROOM-URI CARE AU ADRESA IN CAMPINA
...
SELECT
    ...
    V.PRET
FROM VEHICUL V
WHERE V.ID_SHOWROOM IN
(
    SELECT
        S.ID_SHOWROOM
    FROM SHOWROOM S
    WHERE S.ID_ADRESA IN
    (
        SELECT
            A.ID_ADRESA
        FROM ADRESA A
        WHERE UPPER(A.ORAS) = 'CAMPINA'
    )
);
...
JOIN SHOWROOM S ON VPV.ID_SHOWROOM = S.ID_SHOWROOM
JOIN ANGAJAT_SHOWROOM A ON S.ID_SHOWROOM = A.ID_SHOWROOM
